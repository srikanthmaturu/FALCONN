...
namespace tf_idf_falconn_index {
    ...
    public:
        ...
        falconn::LSHNearestNeighborTable<point_type> *table;

        void construct_table() {
            table = falconn::construct_table<point_type>(dataset, params);
        ...
        typename std::enable_if<std::is_same<point_type, DenseVectorFloat>::value,std::pair<uint64_t, std::vector<std::string>>>::type match(std::string query) {
            ...
            table->find_near_neighbors(query_tf_idf_vector, 0, &nearestNeighbours);
        ...
        typename std::enable_if<std::is_same<point_type, DenseVectorFloat>::value,void> construct_dataset(std::vector<std::string> &data) {
            uint64_t tf_vec_size = pow(2, ngram_length);
